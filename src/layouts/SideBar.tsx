...
const SideBar = () => {
  ...
  const menu = [
    {
      ...
      iconPath: `assets/icons/${
        activeOptionIdx === 0 ? "active" : "inactive"
      }/element-4.svg`,
      option: "Overview",
    },
    {
      ...
      iconPath: `assets/icons/${
        activeOptionIdx === 1 ? "active" : "inactive"
      }/book.svg`,
      option: "Courses",
      dropdown: [
        {
    ...
    },
    {
      path: "/mentors",
      iconPath: `assets/icons/${
        activeOptionIdx === 2 ? "active" : "inactive"
      }/user-octagon.svg`,
      option: "Mentors",
    },
    {
      path: "/message",
      iconPath: `assets/icons/${
        activeOptionIdx === 3 ? "active" : "inactive"
      }/message.svg`,
      option: "Message",
    },
    {
      path: "/setting",
      iconPath: `assets/icons/${
        activeOptionIdx === 4 ? "active" : "inactive"
      }/setting-4.svg`,
      option: "Setting",
    },
  ];
  ...
  return (
    <div
      className={`border-r border-[#F7F7F7] overflow-hidden flex-col align-middle bg-white p-8 h-screen gap-20 z-10 top-0 left-0 transition-all duration-300
  ${
    menuIsOpen ? "display-flex min-w-[28rem]" : "hidden"
  } md:flex md:relative md:min-w-[20rem]`}
    >
      ...
      <div className={`display-flex flex-col gap-2 flex-1 `}>
        {menu.map((option, i) => (
          <Link to={option.path}>
            <div
              onClick={() => {
                setOpenBar(option.option);
              }}
              key={i}
              className={`gap-4 display-flex flex-col rounded-[10px] transition-all duration-300 cursor-pointer ${
                activeOptionIdx === i ? "bg-[#F5F5F7]" : "bg-white"
              }`}
            >
              ...
              <div
                className={`overflow-hidden transition-all duration-300 ${
                  activeOptionIdx === i ? "max-h-40" : "max-h-0"
                }`}
              >
                {option.dropdown
                  ? option.dropdown.map((subOption, j) => (
                      <Link to={subOption.path}>
                        <div
                          onClick={() => {
                            activeSubOptionIdx !== j
                              ? setActiveSubOptionIdx(j)
                              : null;
                          }}
                          key={j}
                          className="gap-2 display-flex flex-col"
                        >
                          <div className="menu-option border-radius flex space-between">
                            <div className="flex gap">
                              <p
      ...
      <div className="w-[196px] h-[150px] bg-[#141522] rounded-[1rem] p-7 flex fixed bottom-6">
        <div className="w-8/12 gap-4 display-flex flex-col">
          ...
          <p className="text-slate-600 font-medium text-nowrap">
            Get 1 Month Free
          </p>
        </div>
